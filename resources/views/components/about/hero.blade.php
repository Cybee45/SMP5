    ...
    <div class="flex flex-col-reverse lg:flex-row items-center gap-y-12 sm:gap-y-16 lg:gap-x-19">
      ...
      <div class="w-full lg:w-5/12 space-y-5 text-left relative -mt-8 sm:-mt-4 lg:-mt-32"
        ...
        <p class="text-sm font-semibold uppercase tracking-wide text-sky-800">
          {{ $aboutHero->subjudul ?? 'Sekolah Menengah Unggulan di Sangatta Utara' }}
        </p>
        <h1 class="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-black leading-tight text-gray-900">
          {{ $aboutHero->judul ?? 'Gali ilmu, ukir prestasi, sambut masa depan gemilang.' }}
        </h1>
        <p class="text-base md:text-lg text-slate-600">
          {{ $aboutHero->deskripsi ?? 'Kita ciptakan lingkungan belajar yang patut diacungi jempol. Siswa bersemangat mendalami ilmu. Gerbang sekolah adalah awal perjalananmu.' }}
        </p>
      </div>
        ...
        <figure class="relative inline-block isolate"
          ...
          @if($aboutHero?->gambar)
            <img src="{{ asset('storage/' . $aboutHero->gambar) }}"
                 ...
                 class="block h-auto object-contain select-none w-[min(68vw,520px)] sm:w-[min(58vw,580px)] lg:w-[540px] xl:w-[580px]" />
          @else
          ...
          <img src="{{ asset('assets/about/globe.png') }}"
               ...
               class="absolute z-20 pointer-events-none select-none drop-shadow-lg animate-float-slow rotate-[340deg] bottom-[10%] right-[63%] w-[clamp(var(--decorMin),20vw,var(--decorMax))]" />
          ...
          <img src="{{ asset('assets/about/light.png') }}"
               ...
               class="absolute z-20 pointer-events-none select-none drop-shadow-lg animate-float-slow top-[30%] left-[63%] w-[clamp(var(--decorMin),24vw,var(--decorMax))]" />
        </figure>
      </div>
